...
Use piping to: 1. Subset the `clinical` data frame to include the columns: `barcode`, `Tobacco.smoking.history`, `total.number.Single.Nucleotide.Variants`. 2. remove NAs in `total.number.Single.Nucleotide.Variants` 3. filter rows to keep only `Current smoker` and `Lifelong Non-smoker` 4. Call this `barcode_TSH_SNV_count_data`
