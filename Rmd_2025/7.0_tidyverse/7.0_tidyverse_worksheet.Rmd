...
**Practice**: Keep rows of penguins that have a bill length of 40.9 mm.
...
ggplot()  + aes(x = bill_length, y = flipper_length) + geom_point() + xlim(30, 60)
```

To the plotting code above, we have added the code `+ xlim(30, 60)`. ggplot will automatically set the appropriate scale on each axis for your plots. In this example, we want to showcase what filtering the dataframe does, so we are forcing each plot to have the same x-axis for easier comparison. Here the minimum is set to 30 and the maximum is set to 60.

Now we can put dataframes in the plotting code and specify the columns we want directly as `x` and `y`.

`ggplot(data=penguins) + aes(x = bill_length_mm, y = flipper_length_mm) + geom_point()  + xlim(30, 60)`
...
ggplot(data=) + aes(x = bill_length_mm, y = flipper_length_mm) + geom_point()  + xlim(30, 60)
```
...
```{r}
# select the columns that contains "Species" OR ends with "mm".
