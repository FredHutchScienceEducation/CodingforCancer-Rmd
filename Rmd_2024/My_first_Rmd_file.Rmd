...
# 11b) Add one new thing or use a different function with the vector heights! 
...
# 12b) put boolean_vector, favorite_foods, and my_numbers into a dataframe
...
# 12c) look at the first few rows of my_df.
```{r}

```

# Utilizing the .csv file dataframe
# 13a) read in the pokemon.csv file as the object called pokemon_df
```{r}

```

# 13b) how many rows and columns are there in pokemon_df?
# 13c) what is the name of the 12th column in the pokemon data frame?
# 13d) what is the name of a column containing character data?
# 13e) what is the name of a column containing numeric or integers?
```{r}

```
# 14a) in your chosen column from 13e (column with numerics or integers)
# are there any NA values?
```{r}

```

# 14b) compute the following quantities with your chosen column containing numerics or integers: max, min, median, mean, standard deviation
```{r}

```

# 14c) how many datapoints are less than the median?
# 14d) how many datapoints are more than or equal to the mean?
```{r}

```

# 14e) how many datapoints are within 30% of the median?
```{r}

```
# 15a) create a vector called pokemon_Total containing the column called Total in the pokemon_df
# 15b) how many pokemon have a total higher than 450?
# 15c) create a vector called pokemon_Defense containing the column called Defense
# in the pokemon_df.
# challenge question 15d) which pokemon has the most Defense? (hint: use the which() function)
```{r}
...
```{r}
# 16a)  I want to find the max time of my vector called sprint_times. I am expecting 17.10 as the result.
sprint_times <- c(10.55, 17.10, 8.24, 11.45)
max(sprint_time)
```

```{r}
# 16b) I want to determine the median of my vector called years. I am expecting 2002.5 as the result
years <- c(2011, 2024, 2022, 1988, 1994)
mean(years)
```

```{r}
# 16c) I want to remove the NAs from my vector called route_numbers and calculate the mean.
route_numbers <- c(25, 12, NA, NA, 99, 84)
